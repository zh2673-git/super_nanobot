...
class LiteLLMProvider(LLMProvider):
    ...
    def __init__(
        ...
        small_model_config: dict | None = None,
    ):
    ...
    async def chat(
        ...
        if self.small_model_mode:
            # 应用 OllamaPilot 小模型优化参数
            temperature = self.small_model_config.get("temperature", temperature)
            max_tokens = self.small_model_config.get("max_tokens", max_tokens)
            ...
            logger.debug(f"小模型模式: temperature={temperature}, max_tokens={max_tokens}, top_p={top_p}")
        ...
        if self.small_model_mode:
            ...
            # num_ctx 通过 extra_body 传递（Ollama 特定参数）
